...
--packfile=<hash>::
	...
	unless `--download-only` or `--stdout` is given. The hash is used to
	determine the name of the temporary file and is arbitrary. The output
	of index-pack is printed to stdout unless `--download-only` or
	`--stdout` is given. Requires `--index-pack-args` unless
	`--download-only` or `--stdout` is given.

--download-only::
	For internal use only, and only valid with `--packfile=<hash>`.
	Download the packfile to a temporary file and write the temporary
	file's path to standard output instead of running `index-pack`.
